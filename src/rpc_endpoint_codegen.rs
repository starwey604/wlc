...
pub(crate) fn fragments(
    ...
    if !profile.rpc_services.is_empty() {
        if !profile.has_rpc_client() {
            config.push_str(
                "  if (runtime_config.rpc_client_enabled) return WL_ERR_NOT_SUPPORTED;\n",
            );
        }
        if !profile.has_rpc_server() {
            ...
            for service in &profile.rpc_services {
                ...
                writeln!(
                    config,
                    "  if (runtime_config.{s}_request_handler != NULL) return WL_ERR_NOT_SUPPORTED;"
                )
                .unwrap();
            }
        }
    ...
    if managed {
        ...
        for service in &profile.rpc_services {
            ...
            let request_value = if profile.has_rpc_server() {
                format!("{req}_value_t request; ")
            } else {
                String::new()
            };
            writeln!(
                state,
                "      struct {{ {request_value}{res}_value_t response; }} {s};"
            )
            .unwrap();
            if profile.has_rpc_server() {
                writeln!(
    ...
    let context = [
        ("M", module),
        ("P", prefix.as_str()),
        ("CLIENT", if profile.has_rpc_client() { "1" } else { "0" }),
    ];
    let bindings = [
    ...
    if managed && profile.has_rpc_client() {
        suffix = render(
