...
static demo_endpoint_t sync_client, sync_server;
...
static void sync_initialize(void) {
  ...
  CHECK(demo_endpoint_init(&sync_client, 31, (wl_clock_t){read_clock, NULL}) == WL_OK);
...
static void sync_close(void) {
  ...
  CHECK(demo_endpoint_close(&sync_client) == WL_OK);
