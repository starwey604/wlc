...
fn generated_rpc_runtime_executes_client_server_and_cache_lifecycles() {
    ...
    assert!(runtime.header.contains("rpc_fixture_runtime_init_checked("));
    assert!(runtime.header.contains("rpc_fixture_runtime_peer_observe("));
    assert!(
        runtime
            .header
            .contains("rpc_fixture_runtime_peer_observation_take(")
    );
    assert!(
        runtime
            .header
...
static wl_rpc_server_request_t handled_request;
...
int wl_tx_take(wl_ctx_t *ctx, wl_tx_handle_t handle,
  ...
  (void)ctx;
  if (handle == 0U || out_result == NULL) return WL_ERR_INVALID_ARG;
  memset(out_result, 0, sizeof(*out_result));
  out_result->state = WL_TX_STATE_SUCCESS;
  return WL_OK;
}

int wl_tx_cancel(wl_ctx_t *ctx, wl_tx_handle_t handle) {
  (void)ctx;
  if (handle == 0U) return WL_ERR_INVALID_ARG;
  ++cancel_calls;
  return WL_OK;
}
...
static int32_t handle_compute(void *user_data,
  ...
  if (server_request == NULL || server_request->generation == 0U ||
      server_request->identity.operation_id == 0U ||
      server_request->identity.request_message_id != COMPUTE_REQUEST_MESSAGE_ID ||
      server_request->identity.response_message_id != COMPUTE_RESPONSE_MESSAGE_ID ||
      server_request->identity.peer_session_id == 0U)
    return -104;
  handled_request = *server_request;
...
static int check_client(wl_ctx_t *ctx, rpc_fixture_runtime_t *runtime,
  ...
  request.tag.length = 1U;
  result = rpc_fixture_compute_client_start(
      ctx, runtime, &request, 100U, 15U);
  if (result.domain != RPC_FIXTURE_RUNTIME_OK ||
      result.detail.rpc.operation_id != 1U || !request.has_operation_id ||
      request.operation_id != 1U)
    return 22;
  {
    const uint32_t sends_with_explicit = send_calls;
    result = rpc_fixture_compute_client_start(
        ctx, runtime, &request, 100U, 16U);
    if (result.domain != RPC_FIXTURE_RUNTIME_RPC_ERROR ||
        result.detail.rpc.operation_id != 1U ||
        result.detail.rpc.rpc_result != WL_RPC_ERR_OPERATION_CONFLICT ||
        send_calls != sends_with_explicit || !request.has_operation_id ||
        request.operation_id != 1U)
      return 24;
  }
  if (compute_request_decode(sent_payload, sent_payload_length,
                             &decoded_request) != WL_CODEC_OK ||
      decoded_request.operation_id != 1U || decoded_request.value != 9U ||
      wl_rpc_client_cancel(client, 1U) != WL_RPC_OK ||
      rpc_fixture_compute_client_release(runtime, 1U) != WL_RPC_OK)
    return 23;

  next_core_result = WL_ERR_BUSY;
  result = rpc_fixture_compute_client_start(
      ctx, runtime, &request, 100U, 17U);
  next_core_result = WL_OK;
  if (result.domain != RPC_FIXTURE_RUNTIME_CORE_ERROR ||
      result.detail.rpc.operation_id != 0U || !request.has_operation_id ||
      request.operation_id != 1U ||
      rpc_fixture_compute_client_inspect(runtime, 1U, &client_result) !=
          WL_RPC_ERR_NOT_FOUND)
    return 25;
  result = rpc_fixture_compute_client_start(
      ctx, runtime, &request, 100U, 18U);
  if (result.domain != RPC_FIXTURE_RUNTIME_OK ||
      result.detail.rpc.operation_id != 1U ||
      wl_rpc_client_cancel(client, 1U) != WL_RPC_OK ||
      rpc_fixture_compute_client_release(runtime, 1U) != WL_RPC_OK)
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_OK || result.detail.rpc.operation_id != 2U ||
      request.has_operation_id || request.operation_id != 0U || direct_active != 0U) return 11;
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_CODEC_ERROR ||
      result.detail.rpc.operation_id != 0U || result.detail.rpc.codec_status != WL_CODEC_ERR_CAPACITY ||
      request.has_operation_id || request.operation_id != 0U)
    return 13;
  if (rpc_fixture_compute_client_inspect(runtime, 3U, &client_result) != WL_RPC_ERR_NOT_FOUND)
    return 14;
  ...
  next_core_result = WL_ERR_BUSY;
  result = rpc_fixture_compute_client_start(
      ctx, runtime, &request, 100U, 35U);
  next_core_result = WL_OK;
  if (result.domain != RPC_FIXTURE_RUNTIME_CORE_ERROR ||
      result.detail.rpc.operation_id != 0U ||
      result.detail.rpc.core_result != WL_ERR_BUSY || direct_active != 0U ||
      request.has_operation_id || request.operation_id != 0U ||
      rpc_fixture_compute_client_inspect(runtime, 4U, &client_result) != WL_RPC_ERR_NOT_FOUND)
    return 15;

  result = rpc_fixture_compute_client_start(
      ctx, runtime, &request, 100U, 40U);
  if (result.domain != RPC_FIXTURE_RUNTIME_OK || result.detail.rpc.operation_id != 5U)
    return 16;
  memset(&response, 0, sizeof(response));
  response.has_operation_id = true;
  response.operation_id = 5U;
  if (compute_response_encode(&response, response_bytes, sizeof(response_bytes),
                              &response_length) != WL_CODEC_OK)
    return 17;
  event.type = WL_EVT_UNRELIABLE_RX;
  ...
  if (rpc_fixture_runtime_get_deadline_hint(runtime, 41U, &deadline) != WL_RPC_OK ||
    ...
    return 19;
  if (rpc_fixture_compute_client_inspect(runtime, 5U, &client_result) != WL_RPC_OK ||
      client_result.state != WL_RPC_CLIENT_TIMED_OUT ||
      rpc_fixture_compute_client_release(runtime, 5U) != WL_RPC_OK)
    return 20;
...
static int check_server(wl_ctx_t *ctx, rpc_fixture_runtime_t *runtime) {
  const uint64_t peer_a = UINT64_C(0xAAA0000000000001);
  const uint64_t peer_b = UINT64_C(0xBBB0000000000002);
  uint8_t request_a[] = {
  ...
  wl_rpc_peer_observation_t peer_observation = {0};
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_OK || !result.event_consumed ||
      ...
      !result.detail.rpc.peer_changed ||
  ...
  if (rpc_fixture_runtime_peer_observation_take(runtime, &peer_observation) !=
          WL_RPC_OK ||
      !peer_observation.changed || peer_observation.previous_session_id != 0U ||
      peer_observation.current_session_id != peer_a)
    return 28;
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_OK ||
      ...
      result.detail.rpc.peer_changed ||
      rpc_fixture_runtime_peer_observation_take(runtime, &peer_observation) !=
          WL_RPC_ERR_NOT_FOUND ||
      handler_calls != 1U) return 3;
  result = dispatch_request(ctx, runtime, request_conflict,
                            sizeof(request_conflict), peer_a, 102U);
  if (result.domain != RPC_FIXTURE_RUNTIME_RPC_ERROR ||
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_OK ||
    ...
    return 9;
  second_request = handled_request;
  memset(&response, 0, sizeof(response));
  response.has_output = true;
  response.output = 10U;
  next_core_result = WL_ERR_BUSY;
  ...
  if (result.domain != RPC_FIXTURE_RUNTIME_OK || response.has_operation_id ||
      response.operation_id != 0U || response.has_status ||
      response.status != SUCCESS ||
  ...
  result = dispatch_request(ctx, runtime, request_three, sizeof(request_three),
                            peer_a, 109U);
  if (result.domain != RPC_FIXTURE_RUNTIME_RPC_ERROR ||
      result.detail.rpc.rpc_result != WL_RPC_ERR_CACHE_FULL || handler_calls != 3U)
    return 12;
  sends_before = send_calls;
  memset(&response, 0, sizeof(response));
  if (rpc_fixture_runtime_poll(runtime, 115U, &progress) != WL_RPC_OK ||
      progress.client_timed_out != 0U ||
      progress.server_pending_expired != 0U ||
      progress.server_cache_expired != 0U)
    return 15;
  if (send_calls != sends_before) return 13;
  handler_result = -8;
  result = dispatch_request(ctx, runtime, request_same, sizeof(request_same),
                            peer_b, 116U);
  if (result.domain != RPC_FIXTURE_RUNTIME_APPLICATION_ERROR ||
      result.detail.rpc.rpc_disposition != WL_RPC_SERVER_NEW ||
      result.detail.rpc.application_result != -8 || handler_calls != 4U ||
      handled_request.identity.peer_session_id != peer_b ||
      !result.detail.rpc.peer_changed ||
      cancel_calls != 0U)
    return 21;
  if (rpc_fixture_runtime_peer_observation_take(runtime, &peer_observation) !=
          WL_RPC_OK ||
      peer_observation.previous_session_id != peer_a ||
      peer_observation.current_session_id != peer_b ||
      peer_observation.discarded.pending_discarded != 0U ||
      peer_observation.discarded.responses_discarded != 2U)
    return 29;
  handler_result = 0;
  return 0;
}
...
int main(void) {
  ...
  rpc_fixture_runtime_instance_t checked_instance = {0};
  rpc_fixture_runtime_config_t disabled_config = {0};
  rpc_fixture_runtime_config_t checked_config = {0};
  rpc_fixture_runtime_requirements_t disabled_requirements = {0};
  rpc_fixture_runtime_requirements_t checked_requirements = {0};
  const rpc_fixture_runtime_storage_t disabled_storage = {NULL, 0U};
  union {
    wl_max_align_t alignment;
    uint8_t bytes[1024];
  } checked_storage_backing = {0};
  rpc_fixture_runtime_storage_t checked_storage = {0};
  rpc_fixture_runtime_init_diagnostic_t diagnostic = {0};
  ...
  if (rpc_fixture_runtime_requirements(&disabled_config,
    ...
    return 1;
  if (rpc_fixture_runtime_config_defaults(&checked_config) != WL_OK)
    return 6;
  checked_config.rpc_client_enabled = 2U;
  checked_storage.data = checked_storage_backing.bytes;
  checked_storage.size = sizeof(checked_storage_backing.bytes);
  if (rpc_fixture_runtime_init_checked(&checked_instance, &checked_config,
                                       &checked_storage, &diagnostic) !=
          WL_ERR_INVALID_ARG ||
      diagnostic.issue != RPC_FIXTURE_RUNTIME_INIT_ROLE_ENABLE ||
      strcmp(diagnostic.field, "rpc_client_enabled") != 0 ||
      strcmp(rpc_fixture_runtime_init_issue_str(diagnostic.issue),
             "role enable must be zero or one") != 0)
    return 7;
  checked_config.rpc_client_enabled = 0U;
  checked_config.rpc_client_slot_count = 1U;
  checked_config.rpc_client_response_capacity = 64U;
  checked_config.rpc_client_enabled = 1U;
  if (rpc_fixture_runtime_requirements(&checked_config,
                                       &checked_requirements) != WL_OK ||
      checked_requirements.storage_size == 0U)
    return 8;
  checked_storage.size = checked_requirements.storage_size - 1U;
  if (rpc_fixture_runtime_init_checked(&checked_instance, &checked_config,
                                       &checked_storage, &diagnostic) !=
          WL_ERR_BUF_TOO_SMALL ||
      diagnostic.issue != RPC_FIXTURE_RUNTIME_INIT_STORAGE_TOO_SMALL ||
      diagnostic.required != checked_requirements.storage_size ||
      diagnostic.provided != checked_storage.size)
    return 9;
  checked_storage.size = sizeof(checked_storage_backing.bytes);
  if (rpc_fixture_runtime_init_checked(&checked_instance, &checked_config,
                                       &checked_storage, &diagnostic) != WL_OK ||
      diagnostic.issue != RPC_FIXTURE_RUNTIME_INIT_OK ||
